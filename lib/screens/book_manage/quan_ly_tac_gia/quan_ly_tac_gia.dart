...
class _QuanLyTacGiaState extends State<QuanLyTacGia> {
  ...
  void _addTacGia() async {
    ...
    for (var tacGia in _tacGias) {
      if (tacGia.tenTacGia.toLowerCase() == tenTacGia.toLowerCase()) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Row(
              children: [
                /* 
                Sử dụng tacGia.tenTacGia thay vì tenTacGia ở dòng 37 
                vì tacGia.tenTacGia đã được chuẩn hóa
                */
                Text(
                  'Tác giả ${tacGia.tenTacGia} đã tồn tại.',
    ...
    final returningId = await dbProcess.insertTacGia(
      TacGia(null, tenTacGia),
    );

    setState(() {
      _tacGias.add(
        TacGiaDto(returningId, tenTacGia, 0),
      );
    });
                        ...
                        : () async {
                            ...
                            await showDialog(
                              ...
                              builder: (ctx) => AlertDialog(
                                ...
                                content: Text('Bạn có chắc xóa Tác giả ${readyDeleteTacGia.tenTacGia}?'),
                                shape: RoundedRectangleBorder(
                                  borderRadius: BorderRadius.circular(10),
                                ),
                                actions: [
                                  ...
                                  FilledButton(
                                    onPressed: () async {
                                      await dbProcess.deleteTacGiaWithMaTacGia(readyDeleteTacGia.maTacGia!);
                                      ...
                                      if (mounted) {
                                        ...
                                        ScaffoldMessenger.of(context).showSnackBar(
                                          SnackBar(
                                            content: Text(
                                              'Đã xóa Tác giả ${readyDeleteTacGia.tenTacGia}.',
                                              textAlign: TextAlign.center,
                                            ),
                                            behavior: SnackBarBehavior.floating,
                                            width: 400,
                    ...
                    onPressed: _selectedRow == -1
                        ...
                        : () async {
                            ...
                            if (updatedTenTacGia != null) {
                              setState(() {
                                _tacGias[_selectedRow].tenTacGia = updatedTenTacGia;
                              });
                              if (mounted) {
                                ScaffoldMessenger.of(context).clearSnackBars();
                                ScaffoldMessenger.of(context).showSnackBar(
